...
class Unflatten(nn.Module):
 ...
 to produce an output of shape (N,32323232 C, H, W).
 """
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            nn.ConvTranspose2d(100, hiddenSize*8, 4, 1, 0, bias=False),
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(hiddenSize, 3, 4, 2, 1, bias=False),
            nn.Tanh())

    def forward(self, z):
        # z, shape=(opt.batch_size, opt.latent_dim)
        z = z.view(z.shape[0],100,1,1)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        hiddenSize = 64
        self.main = nn.Sequential(
            nn.Conv2d(3, hiddenSize, 4, 2, 1, bias=False),
    ...
    # Inference
        ...
        for i in tqdm(range(0, opt.inference_num)):
            save_image(gen_imgs.data[i],
                       "result/images_inference/%d.png" % (i+j*2000),
                       normalize=True)
